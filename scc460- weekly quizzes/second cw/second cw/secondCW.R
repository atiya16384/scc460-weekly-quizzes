...
lotka_volterra_model <- function(rabbit_population, fox_population, alpha, beta, gamma, time_steps, stochastic = FALSE) {
  # Input validation
  if (!is.numeric(alpha) || alpha <= 0) stop("Error: alpha (birth rate) must be a positive number.")
  if (!is.numeric(beta) || beta <= 0) stop("Error: beta (consumption rate) must be a positive number.")
  if (!is.numeric(gamma) || gamma <= 0) stop("Error: gamma (death rate) must be a positive number.")
  if (!is.numeric(rabbit_population[1]) || rabbit_population[1] < 0) stop("Error: initial_rabbits must be a non-negative number.")
  if (!is.numeric(fox_population[1]) || fox_population[1] < 0) stop("Error: initial_foxes must be a non-negative number.")
  if (!is.numeric(time_steps) || time_steps <= 0 || round(time_steps) != time_steps) stop("Error: time_steps must be a positive integer.")
...
det_results <- lotka_volterra_model(rabbit_population, fox_population, alpha, beta, gamma, time_steps, stochastic = FALSE)
det_rabbits <- det_results[[1]]
det_foxes <- det_results[[2]]

# Display the last few values of the deterministic model results
print("Deterministic Model - Last few values:")
print(tail(det_rabbits, n = 5))
print(tail(det_foxes, n = 5))

# Task 2: Run the stochastic model
sto_results <- lotka_volterra_model(rabbit_population, fox_population, alpha, beta, gamma, time_steps, stochastic = TRUE)
sto_rabbits <- sto_results[[1]]
sto_foxes <- sto_results[[2]]

# Display the last few values of the stochastic model results
print("Stochastic Model - Last few values:")
print(tail(sto_rabbits, n = 5))
print(tail(sto_foxes, n = 5))
